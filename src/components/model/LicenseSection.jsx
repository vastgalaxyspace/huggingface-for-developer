...
const LicenseSection = ({ licenseInfo, deploymentRec }) => {
  ...
  const getPermBg = (val) => {
    if (val === true) return 'bg-emerald-50';
    if (val === false) return 'bg-red-50';
    return 'bg-amber-50';
  };

  return (
    <section id="section-license" className="mb-10 fade-in-section scroll-mt-28">
      <div className="flex items-center gap-2 mb-1">
        <Shield className="h-5 w-5 text-[var(--accent)]" />
        <h2 className="text-[1.5rem] font-black tracking-tight text-[var(--text-strong)]">License Analysis</h2>
      </div>
      ...
      <div className={`mb-5 flex items-start gap-3 rounded-2xl border p-5 ${colorMap[licenseInfo.color] || colorMap.gray}`}>
        ...
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--text-main)]">{licenseInfo.details}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="rounded-[22px] border border-[var(--border-soft)] bg-white p-5">
          ...
          <div className="space-y-2.5">
            {permissions.map(p => (
              <div key={p.key} className={`flex items-center justify-between rounded-xl px-3.5 py-2.5 ${getPermBg(p.value)}`}>
        ...
        <div className="rounded-[22px] border border-[var(--border-soft)] bg-white p-5">
          ...
          {deploymentRec && (
            <>
              <p className="mb-3 text-[14px] font-bold text-[var(--text-strong)]">{deploymentRec.message}</p>
